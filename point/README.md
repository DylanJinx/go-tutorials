...
- 第一个函数中，`ele`是对切片某个元素的拷贝，不会对切片里面的原始内容进行修改。
- 第二个函数中，是取得切片的下标，`arr[i]++`是对原始的底层数组进行操作，是可以修改原始内容的。
- 第三个函数中，切片里面存放的数据类型就是指针类型，那么`ele`就是每个元素的地址号的拷贝，`*ele`就是解析地址号里的内容，可以修改原始内容。